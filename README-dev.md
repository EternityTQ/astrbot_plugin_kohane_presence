...
- 白名单私聊先把 `event.plugins_name` 收窄为“原本允许的已激活插件减去精确 excluded_plugins”。`None` / `["*"]` 从 `context.get_all_stars()` 展开；有限列表保持为上界。该值随原 event 保留到延迟 generation；已注册命令只应用该 scope，不停止事件，也不进入 scheduler。
- 白名单普通私聊依次调用 `event.should_call_llm(True)`、`event.stop_event()` 和 `event.clear_result()`。这会保留 stopped 状态，同时清除 `stop_event()` 创建的空 STOP result，避免原始 RespondStage 和 after-send 被误触发。未命中白名单的事件完全不改。
